...
# Camera Zoom
	if event is InputEventMouseButton:
		var camera_spring = $Player_CharacterBody/cameraPivot/SpringArm3D
...
func _physics_process(delta: float) -> void:
	...
	camera_pivot.global_position = lerp(camera_pivot.global_position,%CameraTarget.global_position,0.6 )
	# Rotate to Aim
	#if Input.is_action_just_pressed("aim"):
		#player_character.rotation_degrees.y = camera.rel
